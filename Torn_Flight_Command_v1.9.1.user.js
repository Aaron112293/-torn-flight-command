...
// @version      1.9.1
// @description  Flight Command with dedicated Torn PDA-style Flight tab, live player-market and Torn NPC profit
// @updateURL    https://raw.githubusercontent.com/Aaron112293/-torn-flight-command/main/Torn_Flight_Command_v1.9.1.user.js
// @downloadURL  https://raw.githubusercontent.com/Aaron112293/-torn-flight-command/main/Torn_Flight_Command_v1.9.1.user.js
...
(function () {
    ...
    function installStyles() {
        ...
        style.textContent = `
            ...
            #fc-pda-flight-tab .fc-flight-tab-icon {
            ...
            #fc-pda-flight-tab .fc-flight-tab-label {
                display: block !important;
            ...
            #fc-pda-flight-tab.fc-flight-active {
                outline: 2px solid rgba(74, 190, 255, .95) !important;
                outline-offset: -2px !important;
                box-shadow: inset 0 0 12px rgba(40, 161, 224, .35), 0 0 8px rgba(40, 161, 224, .30) !important;
            }
        `;
    ...
    function visibleLauncherCandidate(element) {
        if (!(element instanceof Element) || element.id === 'fc-pda-flight-tab' || element.closest('#fc-panel')) return false;
        const style = getComputedStyle(element);
    ...
    function nativeNotesLauncher() {
        const candidates = [...document.querySelectorAll('button, [role="button"], a')];
        let best = null;
        let bestScore = -Infinity;

        for (const element of candidates) {
            if (!visibleLauncherCandidate(element)) continue;
            ...
            if (!/^notes$/i.test(text) && !/^notes$/i.test(aria) && !/^notes$/i.test(title)) continue;

            const rect = element.getBoundingClientRect();
            let score = 0;
            if (/^notes$/i.test(text)) score += 5;
            if (/^notes$/i.test(aria)) score += 4;
            if (/^notes$/i.test(title)) score += 3;
            if (rect.top > window.innerHeight * 0.55) score += 4;
            if (rect.bottom > window.innerHeight * 0.72) score += 2;
            if (rect.width <= 140 && rect.height <= 140) score += 1;
    ...
    function setFlightLauncherActive(active) {
        const launcher = document.getElementById('fc-pda-flight-tab');
        if (!launcher) return;
        launcher.classList.toggle('fc-flight-active', Boolean(active));
        launcher.setAttribute('aria-pressed', active ? 'true' : 'false');
    }

    function clearClonedNavigationAttributes(element) {
    ...
    function ensureFlightLauncher() {
        const existing = document.getElementById('fc-pda-flight-tab');
        if (existing?.isConnected) {
            setFlightLauncherActive(panelOpen);
            return existing;
        }

        const notes = nativeNotesLauncher();
        if (!notes?.parentElement) return null;

        installStyles();

        const launcher = notes.cloneNode(false);
        clearClonedNavigationAttributes(launcher);
        launcher.id = 'fc-pda-flight-tab';
        launcher.classList.add('fc-pda-flight-tab');
        launcher.setAttribute('aria-label', 'Flight');
        launcher.setAttribute('title', 'Flight Command');
        launcher.setAttribute('aria-pressed', panelOpen ? 'true' : 'false');

        if (launcher.tagName === 'BUTTON') launcher.setAttribute('type', 'button');
        if (launcher.tagName === 'A') launcher.setAttribute('href', '#');

        launcher.innerHTML = '<span class="fc-flight-tab-icon" aria-hidden="true">✈</span><span class="fc-flight-tab-label">Flight</span>';

        const blockEvent = event => {
            event.stopPropagation();
        };
        launcher.addEventListener('pointerdown', blockEvent, true);
        launcher.addEventListener('touchstart', blockEvent, { capture: true, passive: true });
        launcher.addEventListener('touchend', blockEvent, { capture: true, passive: true });
        ...
        notes.insertAdjacentElement('afterend', launcher);
        setFlightLauncherActive(panelOpen);
        return launcher;
    }

    function toolbarAncestorFor(element) {
        let node = element?.parentElement || null;

        for (let depth = 0; depth < 5 && node; depth++, node = node.parentElement) {
            const rect = node.getBoundingClientRect();
            const controls = node.querySelectorAll('button, [role="button"], a');
            if (controls.length >= 3 && rect.height > 0 && rect.height <= 180 && rect.bottom >= window.innerHeight * 0.70) {
                return node;
            }
        }
    ...
    function closeFlightForOtherPdaTab(event) {
        if (!panelOpen || !(event.target instanceof Element)) return;
        if (event.target.closest('#fc-panel, #fc-pda-flight-tab')) return;

        const notes = nativeNotesLauncher();
        if (!notes) return;

        const toolbar = toolbarAncestorFor(notes);
        const clickedControl = event.target.closest('button, [role="button"], a');
        if (!toolbar || !clickedControl || !toolbar.contains(clickedControl)) return;

        closePanel();
    }

    document.addEventListener('click', closeFlightForOtherPdaTab, true);

    const observer = new MutationObserver(() => {
        ensureFlightLauncher();
    });
