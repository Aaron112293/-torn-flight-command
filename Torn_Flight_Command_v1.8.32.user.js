...
// @updateURL    https://raw.githubusercontent.com/Aaron112293/-torn-flight-command/main/Torn_Flight_Command_v1.8.32.user.js
// @downloadURL  https://raw.githubusercontent.com/Aaron112293/-torn-flight-command/main/Torn_Flight_Command_v1.8.32.user.js
...
(function () {
    ...
    function recordTripPurchase(item, amount, trigger) {
        const trip = loadActiveTripSummary();
        if (!trip || !item || !Number.isFinite(Number(amount)) || Number(amount) <= 0) return;
        const quantity = Math.floor(Number(amount));
        const unitCost = item.cost;
        const unitProfit = selectedProfit(item, profitMode);
        ...
        panel.innerHTML = `
            ...
            <div id="fc-content">
                ...
                <div id="fc-flight-info-row">
                    ...
                    <label class="fc-autopilot-quick-control">
                        AUTOPILOT
                        <input id="fc-autopilot-quick-switch" type="checkbox" ${autopilotEnabled ? 'checked' : ''}>
                    </label>
                </div>
    ...
    function syncFlightPicker() {
        ...
        if (autopilotStatus) {
            autopilotStatus.classList.toggle('paused', autopilotEnabled && reserve.locked);
            if (!autopilotEnabled) {
    ...
    function syncPurchaseControl() {
        ...
        const selectedStock = Number.isFinite(selected?.quantity)
            ? ` · STOCK ${Number(selected.quantity).toLocaleString('en-US')}`
            : '';
        const trip = loadAutoBuyTrip();
        const purchaseWait = remainingAutomationDelaySeconds(trip?.purchaseReadyAt);
        ...
        if (button) {
            ...
            } else if (purchaseWait > 0) {
                button.textContent = selected
                    ? `AUTO BUY IN ${purchaseWait}s: ${selected.name.toUpperCase()}${selectedStock}`
                    : `AUTO BUY IN ${purchaseWait}s`;
            } else if (autoBuyMode === 'manual') {
                button.textContent = `MANUAL BUY: ${manualBuyItem.toUpperCase()}${selectedStock}`;
            } else {
                button.textContent = selected
                    ? `AUTO BUY [${purchaseProfitModeLabel()}]: ${selected.name.toUpperCase()}${selectedStock}`
    ...
    function scheduleAutoReturn(item, amount, trigger) {
        ...
        const requestedDelayMs = randomOverseasActionDelayMs();
    ...
    function renderTornReport() {
        ...
        const activeTrip = loadActiveTripSummary();
        const trips = [
            ...(activeTrip?.destination === selectedSummaryCountry ? [{ trip: activeTrip, active: true }] : []),
            ...loadTripSummaryHistory()
                .filter(trip => trip.destination === selectedSummaryCountry)
                .map(trip => ({ trip, active: false }))
        ].filter(({ trip }) => Array.isArray(trip.purchases) && trip.purchases.length > 0)
            .sort((left, right) => {
                const leftPurchase = left.trip.purchases[left.trip.purchases.length - 1];
                const rightPurchase = right.trip.purchases[right.trip.purchases.length - 1];
                const leftTime = Number(leftPurchase?.purchasedAt || left.trip.requestedAt) || 0;
                const rightTime = Number(rightPurchase?.purchasedAt || right.trip.requestedAt) || 0;
                return rightTime - leftTime;
            });

        const cycles = trips.map(({ trip, active }) => {
            const totals = tripTotals(trip);
            const firstPurchaseAt = Number(trip.purchases[0]?.purchasedAt);
            const date = Number.isFinite(firstPurchaseAt) ? new Date(firstPurchaseAt).toLocaleString() : 'Time unavailable';
            ...
            const outboundDuration = Number.isFinite(outboundStart) && Number.isFinite(landedAt) && landedAt >= outboundStart
                ? landedAt - outboundStart
                : null;
            const returnDuration = Number.isFinite(returnStart) && Number.isFinite(completedAt) && completedAt >= returnStart
                ? completedAt - returnStart
                : null;
            const recordedFlightDuration = [outboundDuration, returnDuration]
                .filter(Number.isFinite)
                .reduce((total, duration) => total + duration, 0);
            const outboundTime = formatTripDuration(trip.outboundStartedAt || trip.requestedAt, trip.landedAbroadAt);
            const returnTime = formatTripDuration(trip.returnStartedAt, trip.completedAt);
            const totalFlightTime = outboundDuration === null && returnDuration === null
                ? '—'
                : formatTripDuration(0, recordedFlightDuration);
            const items = trip.purchases.map(purchase => {
                const itemProfit = Number(purchase.unitProfit);
                return `
                    <div class="fc-summary-cycle-item">
                        <div class="fc-summary-cycle-item-name">Item: ${escapeHtml(purchase.item || 'Unknown item')}</div>
                        Quantity: <b>${Number(purchase.quantity || 0).toLocaleString('en-US')}</b><br>
                        Price: ${money(purchase.unitCost)} each<br>
                        Sale price: ${money(purchase.estimatedResalePrice)} each<br>
                        Profit: <span class="${itemProfit < 0 ? 'fc-report-loss' : 'fc-report-profit'}">${money(itemProfit)} each</span>
                    </div>
                `;
            }).join('');

            return `
                <div class="fc-summary-cycle">
                    <div class="fc-summary-cycle-head">
                        <div class="fc-summary-cycle-location">Location: ${escapeHtml(trip.destination || selectedSummaryCountry)}</div>
                        <div class="fc-summary-cycle-time">${active ? 'CURRENT CYCLE' : 'COMPLETED CYCLE'} · ${escapeHtml(date)}</div>
                    </div>
                    <div class="fc-summary-cycle-items">${items}</div>
                    <div class="fc-summary-cycle-totals">
                        Flight type: ${escapeHtml(trip.flightType || 'Unknown')}<br>
                        Flight time: Outbound ${outboundTime} · Return ${returnTime}<br>
                        Total flight time: ${totalFlightTime}<br>
                        Total items: ${totals.items.toLocaleString('en-US')}<br>
                        Total spent: ${money(totals.spent)}<br>
                        Total profit: <span class="${totals.profit < 0 ? 'fc-report-loss' : 'fc-report-profit'}">${money(totals.profit)}</span>
                    </div>
                </div>
            `;
        }).join('');
        ...
        content.innerHTML = `
            ...
            <div class="fc-summary-section-title">FLIGHT CYCLES</div>
            ${cycles || '<div class="fc-report-empty">A flight cycle will appear here as soon as an item is purchased in Mexico.</div>'}
        `;
