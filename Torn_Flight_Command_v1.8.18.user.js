...
// @version      1.8.18
// @description  Flight Command travel, profit, cash-reserve, and smart purchase controls
// @updateURL    https://raw.githubusercontent.com/Aaron112293/-torn-flight-command/main/Torn_Flight_Command_v1.8.18.user.js
// @downloadURL  https://raw.githubusercontent.com/Aaron112293/-torn-flight-command/main/Torn_Flight_Command_v1.8.18.user.js
...
(function () {
    ...
    function createPanel() {
        ...
        panel.innerHTML = `
            ...
            <div id="fc-content">
                ...
                <div id="fc-purchase-hint">Tap to buy now · Hold for purchase settings</div>
    ...
    function smartPurchaseSelection() {
        ...
        return items
            .filter(item => !item.soldOut
                ...
                && Number.isFinite(selectedProfit(item, 'auto'))
                && selectedProfit(item, 'auto') > 0)
            .sort((left, right) => selectedProfit(right, 'auto') - selectedProfit(left, 'auto'))[0] || null;
    }
        ...
        if (button) {
            ...
            if (smartPurchaseBusy) {
                ...
                button.textContent = selected
                    ? `AUTO BUY: ${selected.name.toUpperCase()}`
                    : 'AUTO BUY: WAITING FOR PROFIT DATA';
            }
        }

        if (status) {
            const selectionProfit = selected ? selectedProfit(selected, autoBuyMode === 'auto' ? 'auto' : profitMode) : null;
            const base = autoBuyMode === 'auto'
                ? (selected
                    ? `Current best: ${selected.name} · ${money(selectionProfit)} each`
                    : 'No profitable in-stock item is currently available')
    ...
    function findReturnControl() {
        return nativeTravelControls().find(control =>
            /\b(?:return|fly|travel)\s+(?:back\s+)?to\s+(?:torn|torn\s+city)\b/i.test(flightControlOwnHint(control))
        ) || null;
    }
    ...
    async function processPendingFlightIntent() {
        ...
        try {
            if (intent.direction === 'RETURNING') {
                ...
                if (!returnControl) throw new Error('A clearly labeled Return to Torn control was not found.');
                returnControl.click();
                recordFlightAction('RETURN_CONTROL_CLICKED', null, { intent, control: controlSnapshot(returnControl) });

                const confirmation = await waitForValue(
                    () => findConfirmationControl('Torn', intent.flightType, true),
                    3500
                );
                if (confirmation) {
                    confirmation.click();
                    ...
                    recordFlightAction('RETURN_CONTROL_CLICKED_NO_CONFIRMATION', null, { intent });
                }
                clearPendingFlightIntent();
                return;
            }
    ...
    function recordSmartPurchase(stage, item = null, amount = null, error = null, extra = {}) {
        lastSmartPurchaseAction = {
            ...
            selectedProfit: item ? selectedProfit(item, autoBuyMode === 'auto' ? 'auto' : profitMode) : null,
            error,
            recordedAt: new Date().toISOString(),
            ...extra
        };
