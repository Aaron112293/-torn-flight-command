...
// @updateURL    https://raw.githubusercontent.com/Aaron112293/-torn-flight-command/main/Torn_Flight_Command_v1.8.31.user.js
// @downloadURL  https://raw.githubusercontent.com/Aaron112293/-torn-flight-command/main/Torn_Flight_Command_v1.8.31.user.js
...
(function () {
    ...
    function renderTornReport() {
        ...
        const purchases = trips.flatMap(({ trip, active }) =>
            (Array.isArray(trip.purchases) ? trip.purchases : []).map(purchase => ({ purchase, trip, active }))
        ).sort((left, right) => Number(right.purchase.purchasedAt) - Number(left.purchase.purchasedAt));

        const totals = purchases.reduce((result, entry) => ({
            amount: result.amount + (Number(entry.purchase.quantity) || 0),
            spent: result.spent + (Number(entry.purchase.totalCost) || 0),
            profit: result.profit + (Number(entry.purchase.totalProfit) || 0)
        }), { amount: 0, spent: 0, profit: 0 });

        const purchaseList = purchases.map(({ purchase, trip, active }) => {
            const profit = Number(purchase.totalProfit);
            const purchasedAt = Number(purchase.purchasedAt);
            const date = Number.isFinite(purchasedAt) ? new Date(purchasedAt).toLocaleString() : 'Time unavailable';
            ...
            return `
                <div class="fc-summary-item-row">
                    <div class="fc-summary-item-name">${escapeHtml(purchase.item || 'Unknown item')}</div>
                    <div class="fc-report-trip-meta">${active ? 'CURRENT TRIP' : 'COMPLETED TRIP'} · ${escapeHtml(date)}</div>
                    Amount purchased: <b>${Number(purchase.quantity || 0).toLocaleString('en-US')}</b><br>
                    Cost: ${money(purchase.unitCost)} each · ${money(purchase.totalCost)} total<br>
                    Estimated resale: ${money(purchase.estimatedResalePrice)} each<br>
                    Calculated profit: ${money(purchase.unitProfit)} each<br>
                    Total calculated profit: <span class="${profit < 0 ? 'fc-report-loss' : 'fc-report-profit'}">${money(profit)}</span><br>
                    Flight: ${escapeHtml(trip.flightType || 'Unknown')} · Outbound ${outboundTime} · Return ${returnTime}
                </div>
            `;
        }).join('');
        ...
        content.innerHTML = `
            ...
            <div class="fc-summary-compact">
                ${escapeHtml(selectedSummaryCountry)} purchases: ${purchases.length.toLocaleString('en-US')}<br>
                Item amount: ${totals.amount.toLocaleString('en-US')} · Total spent: ${money(totals.spent)}<br>
                Overall calculated profit: <span class="${totals.profit < 0 ? 'fc-report-loss' : 'fc-report-profit'}">${money(totals.profit)}</span>
            </div>
            <div class="fc-summary-section-title">PURCHASE LIST</div>
            ${purchaseList || '<div class="fc-report-empty">This list will populate when an item is purchased in Mexico.</div>'}
        `;
